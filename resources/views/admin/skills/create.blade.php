...
<div class="card">
    <form action="{{ route('admin.skills.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" required value="{{ old('name') }}" placeholder="Contoh: HTML5">
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="number" name="percentage" required value="{{ old('percentage') }}" min="0" max="100" placeholder="Contoh: 85">
            @error('percentage')
                <small style="color: red;">{{ $message }}</small>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <select name="category" required style="width: 100%; padding: 0.8rem; border: 2px solid #e0e0e0; border-radius: 8px; font-family: 'Poppins', sans-serif; font-size: 1rem;">
                ...
                <option value="hard" {{ old('category') == 'hard' ? 'selected' : '' }}>Hard Skill</option>
                <option value="soft" {{ old('category') == 'soft' ? 'selected' : '' }}>Soft Skill</option>
            </select>
            @error('category')
                <small style="color: red;">{{ $message }}</small>
            @enderror
