...
<div class="container-fluid px-4">
    <div class="my-4">
        <h1 class="h3 mb-0">Add New Item for: {{ $hobby->name }} {{ $hobby->icon }}</h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.hobbies.index') }}">Hobbies</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.hobby-items.index', $hobby) }}">Items</a></li>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('admin.hobby-items.store', $hobby) }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('content') is-invalid @enderror" 
                              ...
                              name="content" 
                              rows="4"
                              placeholder="Description, review, or recipe">{{ old('content') }}</textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('metadata') is-invalid @enderror font-monospace" 
                              ...
                              name="metadata" 
                              rows="5"
                              placeholder='{"year": 2010, "genre": "Sci-Fi", "rating": 9.5}'>{{ old('metadata') }}</textarea>
                    @error('metadata')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="alert alert-info">
                    <strong>Current Hobby Type:</strong> <span class="badge bg-primary">{{ $hobby->type }}</span><br>
